...
def _hash_report(certificate, styles):
    ...
    rows = [
        ...
        ['Acquired (IST)', f"{evidence.acquisition_timestamp:%d/%m/%Y %H:%M:%S}"],
        ...
        ['Last verified (IST)',
         f"{evidence.last_verified_at:%d/%m/%Y %H:%M:%S}" if evidence.last_verified_at else '—'],
...
def _custody_annexure(certificate, styles):
    ...
    for event in evidence.custody_events.order_by('sequence'):
        body.append([
            ...
            Paragraph(f"{event.timestamp:%d/%m/%Y %H:%M:%S}", styles['field']),
...
def _decorate(canvas, doc, certificate):
    ...
    canvas.drawCentredString(
        ...
        f"{certificate.generated_at:%d/%m/%Y %H:%M} IST",
    )
