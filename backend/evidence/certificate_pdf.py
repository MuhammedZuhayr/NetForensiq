...
def render_certificate_pdf(certificate, output_path=None):
    ...
    # Provenance banner, before anything else on the page.
    #
    # A generated capture and a seized one produce byte-identical certificates,
    # so without this a demonstration PDF is indistinguishable from a statutory
    # declaration over real evidence. It is stated at the top, in the strongest
    # terms the document uses, because a reader who stops after the first
    # paragraph must still have been told.
    evidence = certificate.evidence
    if evidence.is_demonstration_only:
        story.append(Paragraph(
            '<b>SYNTHETIC DATA — NOT EVIDENCE.</b> The file certified below was '
            'generated by this software for demonstration. It was never observed '
            'on any network, no seizure took place, and this document is not a '
            'certificate in any proceeding.',
            ParagraphStyle('synthetic', parent=styles['field'],
                           textColor=colors.white,
                           backColor=colors.HexColor('#B3261E'),
                           borderPadding=6, spaceAfter=8),
        ))
    elif evidence.provenance == evidence.Provenance.REFERENCE:
        story.append(Paragraph(
            '<b>REFERENCE CAPTURE.</b> Real traffic obtained from a published '
            'research corpus, used to demonstrate and validate the analysis. It '
            'was not seized in any investigation and is not evidence in any '
            f'matter. {evidence.provenance_detail}',
            ParagraphStyle('reference', parent=styles['field'],
                           textColor=colors.HexColor('#7A4F00'),
                           borderColor=colors.HexColor('#7A4F00'),
                           borderWidth=0.6, borderPadding=5, spaceAfter=8),
        ))
    elif evidence.provenance == evidence.Provenance.UNATTESTED:
        story.append(Paragraph(
            '<b>ORIGIN NOT DECLARED.</b> No provenance was recorded for this '
            'file at intake and no manifest accompanied it. Its origin must be '
            'established by other means before this certificate is relied upon.',
            ParagraphStyle('unattested', parent=styles['field'],
                           textColor=colors.HexColor('#7A4F00'),
                           borderColor=colors.HexColor('#7A4F00'),
                           borderWidth=0.6, borderPadding=5, spaceAfter=8),
        ))
